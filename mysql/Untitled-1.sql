...
5.SELECT e.employee_id, e.first_name, e.last_name, e.salary, m.first_name AS manager_first_name, m.last_name AS manager_last_name, m.salary AS manager_salary
FROM employees e
LEFT JOIN employees m ON e.manager_id = m.employee_id


6.SELECT m.employee_id AS manager_id, m.first_name AS manager_first_name, m.last_name AS manager_last_name, COUNT(e.employee_id) AS employee_count
FROM employees e
RIGHT JOIN employees m ON e.manager_id = m.employee_id
GROUP BY m.employee_id
ORDER BY employee_count DESC

7. SELECT d.department_id, d.department_name, COUNT(e.employee_id) AS employee_count
FROM departments d
LEFT JOIN employees e ON d.department_id = e.department_id
GROUP BY d.department_id

8. SELECT YEAR(hire_date) AS hire_year, COUNT(employee_id) AS employee_count
FROM employees
...
12. SELECT first_name, CONCAT('(', SUBSTR(phone_number, 1, 3), ')-',
                           SUBSTR(phone_number, 4, 3), '-',
                           SUBSTR(phone_number, 7, 4)) AS formatted_phone
FROM employees;
...
17.SELECT *
FROM employees
WHERE EXTRACT(YEAR FROM hire_date) LIKE '8%';

18. SELECT first_name, CONCAT(REVERSE(last_name), ', ', REVERSE(first_name)) AS name_reverse
FROM employees;
...
20. SELECT m.first_name AS manager_first_name, m.last_name AS manager_last_name, e.first_name AS employee_first_name, e.last_name AS employee_last_name
FROM employees e
